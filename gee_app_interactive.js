// ============================================================================
// GEE_ET INTERACTIVE APP
// Deployable Earth Engine App for Thailand Daily ET, GPP, WUE (Jan 2026)
// Displays live tiles without needing to copy-paste URLs
// ============================================================================
...
// Get daily means for visualization
var etDaily = dailyPredictions2026.select('ET_daily_mm').mean();
var gppDaily = dailyPredictions2026.select('GPP_daily_gC_m2').mean();
var wueDaily = dailyPredictions2026.select('WUE_gC_kgH2O').mean();
...
map.addLayer(etDaily.clip(roi).visualize(etVis), {}, 'ET Daily Average');
map.addLayer(gppDaily.clip(roi).visualize(gppVis), {opacity: 0.7}, 'GPP Daily Average');
map.addLayer(wueDaily.clip(roi).visualize(wueVis), {opacity: 0.7}, 'WUE Daily Average');

// Create UI
var panel = ui.Panel({
  widgets: [
    ui.Label('🌏 GEE_ET Interactive App', {fontSize: '20px', fontWeight: 'bold'}),
    ui.Label('Thailand Daily ET, GPP, WUE - January 2026'),
    ui.Label('MLR trained on MODIS 2025 + ERA5 predictors'),
    ui.Label(''),
    ui.Label('📊 Data:', {fontWeight: 'bold'}),
    ui.Label('ET: Evapotranspiration (mm/day)'),
    ui.Label('GPP: Gross Primary Productivity (g C/m²/day)'),
    ui.Label('WUE: Water Use Efficiency (g C/kg H₂O)'),
    ui.Label(''),
    ui.Label('🎯 Use layer toggle (top right) to switch layers'),
    ui.Label('🎚️ Use opacity slider to blend layers'),
  ],
  style: {width: '300px', padding: '10px'}
});

ui.root.widgets().reset([panel, map]);
